...
app.post('/api/calculate-signal', async (req, res) => {
  try {
    const { pair, timeframe, licenseKey, candles: clientCandles } = req.body || {};
    if (REQUIRE_LICENSE) {
    ...
    for (const sym of guessBrokerSymbols(pair)) {
      ...
      if (validateCandles(serverCandles) && serverCandles.length >= 35) { candles = serverCandles; break; }
    }

    const tfSeconds = tickStore.TIMEFRAME_SECONDS[timeframe] || 60;
    const staleLimitMs = Math.max(2 * tfSeconds * 1000, 10 * 60 * 1000);
    const clientValid = validateCandles(clientCandles) && clientCandles.length >= 35;
    ...
    if (!validateCandles(candles) || candles.length < 35) {
